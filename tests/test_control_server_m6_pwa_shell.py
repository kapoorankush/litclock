...
DESIGN_MD_PRESENT = (REPO_ROOT / "DESIGN.md").exists()
_needs_design_md = pytest.mark.skipif(
    not DESIGN_MD_PRESENT,
    reason="DESIGN.md is maintainer-local (gitignored, excluded from the public repo)",
)
...
class TestDocumentationPins:
    ...
    The DESIGN.md pins only run where the file exists — it is
    maintainer-local (excluded from the public repository), so public
    contributors and the public repo's CI skip them; the maintainer's
    checkout still enforces them."""
